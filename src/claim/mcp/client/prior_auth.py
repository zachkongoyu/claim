...
class PriorAuthClient:
    ...
    async def __aenter__(self) -> "PriorAuthClient":
        await self.connect()
        return self
    ...
    async def predict_required(self, payload: dict[str, Any]) -> dict[str, Any]:
        return await self._call_tool("predict_required", {"payload": payload})

    async def fetch_rules(self, payload: dict[str, Any]) -> dict[str, Any]:
        return await self._call_tool("fetch_rules", {"payload": payload})

    async def build_request(self, payload: dict[str, Any]) -> dict[str, Any]:
        return await self._call_tool("build_request", {"payload": payload})

    async def track_status(self, payload: dict[str, Any]) -> dict[str, Any]:
        return await self._call_tool("track_status", {"payload": payload})
